...
import { FAUCET_SLEEP_RANGE, SHUFFLE_WALLETS, SOMNIA_TESTNET_EXPLORER_URL } from '@data/config'
  ...
  try {
    ...
    for (const wallet of wallets) {
      try {
        ...
        if (!response.ok) {
          logger.warn(`(${wallet.id}) Response is not ok: ${response.status} ${response.statusText}\n%o`, responseData)
        } else {
